  ...
  return (
    ...
    >
      <Container>
        ...
        <figure className="relative mx-auto mt-12 max-w-[42rem] rounded-[1.75rem] border border-black/[0.05] bg-[#efe4d6] px-8 py-14 shadow-[0_6px_40px_-16px_rgba(0,0,0,0.1)] md:mt-14 md:px-14 md:py-16 lg:max-w-[44rem]">
          <span
        ...
        <div className="mx-auto mt-14 flex max-w-2xl flex-wrap justify-center gap-5 md:mt-16 md:gap-7">
          {testimonial.memberPhotos.map((p, i) => (
            <img
              key={p.seed}
              src={`https://i.pravatar.cc/128?img=${i + 10}`}
              alt={p.alt}
              className="h-[3.75rem] w-[3.75rem] rounded-full border-[3px] border-white object-cover shadow-[0_4px_16px_-4px_rgba(0,0,0,0.15)] md:h-16 md:w-16"
              loading="lazy"
            />
          ))}
        </div>
